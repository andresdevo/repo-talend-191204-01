...
package repo_talend_191204_01.primerjob_0_1;
 ...
 * @status
 */
public class primerjob implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "primerjob.log");
	}
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(primerjob.class);
	...
	private final String jobName = "primerjob";
	...
	JobStructureCatcherUtils talendJobLog = new JobStructureCatcherUtils(jobName, "_XkZHoBaPEeqcROoZHQkefA", "0.1");
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(primerjob.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_REPO_TALEND_191204_01_primerjob = new byte[0];
		static byte[] commonByteArray_REPO_TALEND_191204_01_primerjob = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_REPO_TALEND_191204_01_primerjob.length) {
					if (length < 1024 && commonByteArray_REPO_TALEND_191204_01_primerjob.length == 0) {
						commonByteArray_REPO_TALEND_191204_01_primerjob = new byte[1024];
					} else {
						commonByteArray_REPO_TALEND_191204_01_primerjob = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_REPO_TALEND_191204_01_primerjob, 0, length);
				strReturn = new String(commonByteArray_REPO_TALEND_191204_01_primerjob, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_REPO_TALEND_191204_01_primerjob) {
...
///////////////////////    			
				...
				if (enableLogStash) {
					talendJobLog.addComponentMessage("tFixedFlowInput_2", "tFixedFlowInput");
					talendJobLogProcess(globalMap);
				}
...
///////////////////////		
					...
					if (row2.mensaje != null) { //

						strBuffer_tLogRow_2.append(String.valueOf(row2.mensaje));
...
///////////////////////    			
				...
				if (enableLogStash) {
					if (resourceMap.get("inIterateVComp") == null || !((Boolean) resourceMap.get("inIterateVComp"))) {
						...
						talendJobLog.addConnectionMessage("tFixedFlowInput_2", "tFixedFlowInput", false, "output",
								"row2", talend_statebean.getNbLine(), talend_statebean.getStartTime(),
								talend_statebean.getEndTime());

						talendJobLog.addConnectionMessage("tLogRow_2", "tLogRow", true, "input", "row2",
								talend_statebean.getNbLine(), talend_statebean.getStartTime(),
								talend_statebean.getEndTime());
						talendJobLogProcess(globalMap);
	...
	public static void main(String[] args) {
		final primerjob primerjobClass = new primerjob();

		int exitCode = primerjobClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'primerjob' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'primerjob' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = primerjob.class.getClassLoader()
					.getResourceAsStream("repo_talend_191204_01/primerjob_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = primerjob.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : primerjob");
		}
		if (enableLogStash) {
...
/************************************************************************************************
 * 59642 characters generated by Talend Cloud Big Data on the 4 de diciembre de
 * 2019 15:57:05 CET
 ************************************************************************************************/